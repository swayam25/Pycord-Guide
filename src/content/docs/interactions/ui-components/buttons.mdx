...
import ButtonResponsePayload from "$payloads/interactions/ui-components/buttons-button-response.json";
...
Buttons weren't the only update to the interactions system in Discord. Discord also added [Select Menus](/interactions/ui-components/dropdowns/) and [Modal Dialogs](/interactions/ui-components/modal-dialogs/), both of which work very similarly to buttons.
...
<NewDiscordComponent messages={[ButtonResponsePayload]} />
...
![Different Button Styles]($images/interactions/button-styles.webp)
...
This can be done by specifying the `row` argument in the [`discord.ui.button`](https://docs.pycord.dev/en/stable/api/ui_kit.html#discord.ui.button) decorator.
...
<Aside type="note">If the `on_timeout` coroutine is not present, the components will simply stop working after the specified time.</Aside>
